...
class M_Comments extends Conexion{
    ...
    public function getCommentsUser($user){
        $query = parent::con()->query("SELECT * FROM users WHERE username = '$user' and status = '0'");
    ...
    public function insertComment(Comments $comment){
        $sentencia = parent::con()->query("INSERT INTO comments (name, email, comment, post_Id) VALUES ('$comment->getName(), $comment->getComment(), $comment->getEmail(), $comment->getPost_id(), $comment->getCreated_at(), $comment->getStatus()')");
